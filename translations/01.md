...
目前，我们只能处理整数，因此属性 `stack` 被注释为一个整数列表。我们还修改了 `__repr__` 方法，以便在任何给定时间点都能轻松查看栈中的内容。
...
下面的[练习](#练习)会让你尝试实现我们下一步要实现的几个功能，那么，就来看看这些吧。

## 练习

- 在 `TokenType` 和 `BytecodeType` 中实现 `__repr__` 方法，使打印标记和字节码的输出稍短。
